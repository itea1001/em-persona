...
Adapted from lm-evaluation-harness AIME utils.
"""

import re


def process_results(response: str, target: str) -> bool:
    """Extract answer from response and compare to target. Returns True if equivalent."""
    ...
    return is_equiv(answer, str(target))
...
def fix_fracs(string):
    """Normalize \\frac shorthand to full form."""
...
def fix_sqrt(string):
    """Normalize \\sqrt shorthand to full form."""
...
def strip_string(string):
    ...
    # Remove short variable assignments like "k = "
    if len(string.split("=")) == 2:
